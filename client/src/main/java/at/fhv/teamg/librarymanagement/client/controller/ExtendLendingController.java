...
public class ExtendLendingController implements Initializable, Parentable<MediaDetailsController> {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        LOG.debug("Initialized LendingExtensionController");
        loadAdditionalData();
        ...
        this.btnReturn.setOnAction(e -> {
            ...
            try {
                switch (currentMediumType) {
                    case BOOK:
                        response = client.extendBook(builder.build());
                        break;
                    case DVD:
                        response = client.extendDvd(builder.build());
                        break;
                    case GAME:
                        response = client.extendGame(builder.build());
                        break;
                    default:
            ...
            if (response != null) {
                if (response.getType().equals(MessageDto.MessageType.SUCCESS)) {
                    AlertHelper.showAlert(
                        ...
                        this.extendLendingPane.getScene().getWindow(),
                ...
                AlertHelper.showAlert(
                    ...
                    this.extendLendingPane.getScene().getWindow(),
                    "Extend lending failed",
                    "Something went wrong. Extend lending failed."
                );
            }
    ...
    private void loadAdditionalData() {
        try {
            allCustomers = RemoteClient.getInstance().getAllCustomers();
